...
def create_masked_lm_predictions(tokens, masked_lm_prob,
  ...
  for index in cand_indexes:
    ...
    if index in covered_indexes:
       continue
    #15% mask five gram
    if rng.random() >=0.85 and index<len(cand_indexes)-4:
        for ind in [index,index+1,index+2,index+3,index+4]:
            ...
            masked_lms.append(MaskedLmInstance(index=index, label=tokens[ind]))

    #20% mask four gram
    elif rng.random() >= 0.65 and index<len(cand_indexes)-3:
    ...
    #30% mask trigram
    elif rng.random() >=0.35 and index<len(cand_indexes)-2:
    ...
    #20% mask bigram
    elif rng.random() >=0.15 and index<len(cand_indexes)-1:
        for ind in [index,index+1,index+2]:
            if ind in covered_indexes:
                continue
            covered_indexes.add(ind)
